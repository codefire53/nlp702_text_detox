from transformers.modeling_outputs import BaseModelOutputWithPast, BaseModelOutputWithCrossAttentions, CausalLMOutputWithPast, CausalLMOutputWithCrossAttentions
...
def forward_contrast_vector_bloom(
        self,
...
def forward_contrast_vector(
        ...
        if (isinstance(self, LlamaModel) and (hasattr(self, '_use_flash_attention_2') and self._use_flash_attention_2)) or (isinstance(self, MistralModel) and self.config._attn_implementation == "flash_attention_2"):
            # 2d mask is passed through the layers
            attention_mask = attention_mask if (attention_mask is not None and 0 in attention_mask) else None
        elif ((isinstance(self, LlamaModel) and (hasattr(self, '_use_sdpa') and self._use_sdpa)) or (isinstance(self, MistralModel) and self.config._attn_implementation == "sdpa")) and not output_attentions:
...
class ConstrastVecBloomForCausalLM(BloomForCausalLM):
    def __init__(self, config):
        ...
        self.transformer.forward = partial(forward_contrast_vector_bloom, self.transformer)
