...
def run_evaluation(
    ...
    df_languages = pd.read_json(args.languages, convert_dates=False, lines=True)
    df_languages = df_languages[["id", "text"]]
    df_languages.set_index("id", inplace=True)
    df_languages.rename(columns={"text": "language"}, inplace=True)
    df = df.join(df_languages)
    ...
    for lang in tqdm(languages):
        df_subset = df[df['language']==lang]

        result = evaluator(
            original_texts=df_subset["input"].tolist(),
            rewritten_texts=df_subset["prediction"].tolist(),
            references=df_subset["reference"].tolist() or None,
        )

        aggregated = {measure: np.mean(values).item() for measure, values in result.items()}
        args.output.write(f"Language: {lang}\n")
